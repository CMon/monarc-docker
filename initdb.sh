...
MYSQL_BASE_CALL="mysql --skip-ssl -h $DB_HOST -u $DB_USER -p$DB_PASSWORD"
...
while [ $retry -lt $MAX_RETRIES ]; do
    $MYSQL_BASE_CALL -D $DB_CLI_NAME -e "SELECT 1;" >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        connected=1
...
echo "GRANT ALL PRIVILEGES ON * . * TO 'monarc'@'%'" | mysql --skip-ssl -h $DB_HOST -u root -p$DB_ROOT_PASSWORD

echo "Create $DB_COMMON_NAME db"
echo "CREATE DATABASE IF NOT EXISTS $DB_COMMON_NAME DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;" | $MYSQL_BASE_CALL

echo "Initialize the common database"
$MYSQL_BASE_CALL $DB_COMMON_NAME < $PATH_TO_MONARC/db-bootstrap/monarc_structure.sql
$MYSQL_BASE_CALL $DB_COMMON_NAME < $PATH_TO_MONARC/db-bootstrap/monarc_data.sql

echo "Create $DB_CLI_NAME db"
echo "DROP DATABASE $DB_CLI_NAME;" | $MYSQL_BASE_CALL
echo "CREATE DATABASE $DB_CLI_NAME DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;" | $MYSQL_BASE_CALL
